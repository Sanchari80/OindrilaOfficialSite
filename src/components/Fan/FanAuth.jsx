...
/* ══ Film Button ══ */
const FilmButton = ({ onClick, children, loading, danger }) => (
  <button type="button" onClick={onClick} disabled={loading}
    style={{
      ...
      background: danger ? 'rgba(192,57,43,0.12)' : loading ? 'rgba(201,168,76,0.4)' : '#c9a84c',
      border: danger ? '1px solid rgba(192,57,43,0.35)' : '1px solid #c9a84c',
      color: danger ? '#e74c3c' : '#0a0a08',
      borderRadius:2, transition:'all 0.2s', opacity: loading ? 0.75 : 1,
    }}
    onMouseEnter={e => { if (!loading && !danger) e.currentTarget.style.background='#e8c97a'; }}
    onMouseLeave={e => { if (!loading && !danger) e.currentTarget.style.background='#c9a84c'; }}
  >
    {loading ? (
      <>
...
const FanAuth = ({ onAuthSuccess }) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      toast.success(`স্বাগতম, ${displayName.trim()}! 🎬`);
    } catch (err) {
      const msg =
      ...
      <div style={{ width:'100%', maxWidth:340 }}>
        ...
        <div style={{ border:'1px solid rgba(201,168,76,0.4)', borderRadius:4, overflow:'hidden', background:'#0f0f0c', boxShadow:'0 20px 50px rgba(0,0,0,0.7)' }}>
          ...
          <div style={{ padding:'30px 26px', textAlign:'center' }}>
            ...
            <h2 style={{ fontFamily:"'Bebas Neue', sans-serif", fontSize:'1.4rem', letterSpacing:'0.1em', color:'#e8c97a', marginBottom:4 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              <a href="/community"
                style={{ display:'block', padding:'11px', textAlign:'center', textDecoration:'none', fontFamily:"'Bebas Neue', sans-serif", fontSize:'0.82rem', letterSpacing:'0.25em', background:'#c9a84c', color:'#0a0a08', borderRadius:2, transition:'background 0.2s' }}
