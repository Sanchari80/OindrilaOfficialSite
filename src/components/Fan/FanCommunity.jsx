...
import { LogOut, Send, Image as ImageIcon, Users, X, Upload, Smile } from 'lucide-react';
...
const FontImport = () => (
  <style>{`
    ...
    @keyframes emojiPop { from { opacity:0; transform:scale(0.9) translateY(6px); } to { opacity:1; transform:scale(1) translateY(0); } }
...
const SprocketRow = ({ count = 8 }) => (
  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-around', padding:'0 8px' }}>
    {Array.from({ length: count }).map((_,i) => (
      <div key={i} style={{ width:11, height:7, background:'#1c1409', border:'1px solid rgba(201,168,76,0.35)', borderRadius:2 }} />
    ))}
  </div>
);
...
const MessageBubble = ({ msg, isOwn }) => {
  ...
  const time = msg.timestamp
    ? new Date(msg.timestamp?.toDate ? msg.timestamp.toDate() : msg.timestamp).toLocaleTimeString('en-BD', { hour:'2-digit', minute:'2-digit' })
    : '';
    ...
    <div style={{ display:'flex', flexDirection: isOwn ? 'row-reverse' : 'row', gap:8, alignItems:'flex-end', animation:'fadeInUp 0.3s ease', maxWidth:'85%', alignSelf: isOwn ? 'flex-end' : 'flex-start' }}>
      ...
      <div style={{ maxWidth:'100%' }}>
        ...
        <div style={{
          ...
          <div style={{ position:'relative', padding: msg.imageUrl ? '6px' : '8px 12px', background: isOwn ? 'rgba(201,168,76,0.12)' : 'rgba(255,255,255,0.08)' }}>
            {/* Image message */}
            {msg.imageUrl && (
              <img src={msg.imageUrl} alt="sent" style={{ maxWidth:220, maxHeight:200, display:'block', borderRadius:6, objectFit:'cover', marginBottom: msg.text ? 4 : 0 }} />
            )}
            {/* Text */}
            {msg.text && (
...
const EmojiPicker = ({ onSelect, onClose }) => (
  <div style={{
    ...
    background:'#1a1a12', border:'1px solid rgba(201,168,76,0.3)',
    borderRadius:6, padding:10, width:260, zIndex:50,
    boxShadow:'0 8px 30px rgba(0,0,0,0.6)',
    animation:'emojiPop 0.2s ease',
  }}>
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
      <span style={{ fontFamily:"'Bebas Neue', sans-serif", fontSize:'0.65rem', letterSpacing:'0.2em', color:'rgba(201,168,76,0.5)' }}>EMOJIS</span>
      <button onClick={onClose} style={{ background:'none', border:'none', color:'rgba(201,168,76,0.4)', cursor:'pointer', padding:2, lineHeight:1 }}><X size={12}/></button>
    </div>
    <div style={{ display:'flex', flexWrap:'wrap', gap:4 }}>
      {EMOJIS.map((e, i) => (
        <button key={i} onClick={() => onSelect(e)}
          style={{ width:30, height:30, background:'transparent', border:'1px solid rgba(201,168,76,0.1)', borderRadius:4, cursor:'pointer', fontSize:'1rem', display:'flex', alignItems:'center', justifyContent:'center', transition:'background 0.15s' }}
          onMouseEnter={ev => ev.currentTarget.style.background='rgba(201,168,76,0.15)'}
...
const FanCommunity = () => {
  ...
  const [msgBg,        setMsgBg]        = useState(null);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const msgs = snap.docs.map(d => ({ ...d.data(), id: d.id, _fromFirestore: true }));
      setMessages(msgs);
      if (!histLoaded) setHistLoaded(true);
    });
    return () => unsub();
  }, [user]);
  ...
  useEffect(() => {
    if (!user) return;
    const socket = io(SOCKET_URL, { transports:['websocket','polling'], reconnection:true, reconnectionAttempts:10 });
    ...
    /* real-time messages from OTHER users — add to list (Firestore onSnapshot handles own) */
    socket.on('chat:message', (msg) => {
      if (msg.type === 'system') {
        setMessages(prev => [...prev, { ...msg, _realtime: true }]);
        return;
      }
      // only add if it's from another user (own messages saved + shown via Firestore)
      if (msg.uid !== myUidRef.current) {
        setMessages(prev => {
          // avoid duplicate if Firestore already added it
          if (prev.some(m => m.id === msg.id)) return prev;
          return [...prev, { ...msg, _realtime: true }];
        });
      }
    });
  ...
  const saveToFirestore = async (msgData) => {
    try {
  ...
  const sendMessage = useCallback(async () => {
    ...
    await saveToFirestore(msgData);

    // broadcast to others via socket
    socketRef.current.emit('chat:send', { ...msgData, id: Date.now() + Math.random() });
  ...
  const handlePresetSelect = (value) => { setChatBg(value); setMsgBg(null); setShowBgPicker(false); };
  const handleLogout = async () => { await signOut(auth); navigate('/fan-zone'); };
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', background:'#12120e', color:'#f2ead8', position:'relative', zIndex:5, isolation:'isolate' }}>
      ...
      <div style={{ background:'#1a1a12', borderBottom:'1px solid rgba(201,168,76,0.15)', flexShrink:0 }}>
        <div style={{ padding:'5px 0', borderBottom:'1px solid rgba(201,168,76,0.1)' }}>
          <SprocketRow count={12} />
        </div>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 14px', flexWrap:'wrap', gap:6 }}>
          <div>
            <h1 style={{ fontFamily:"'Bebas Neue', sans-serif", fontSize:'1.2rem', letterSpacing:'0.2em', color:'#c9a84c', margin:0, lineHeight:1 }}>🎬 FAN COMMUNITY</h1>
            <p style={{ fontFamily:'monospace', fontSize:'0.45rem', letterSpacing:'0.2em', color:'rgba(201,168,76,0.35)', margin:'3px 0 0', textTransform:'uppercase' }}>Oindrila Sen · Official Fan Zone</p>
          </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:4 }}>
              ...
              <span style={{ fontFamily:'monospace', fontSize:'0.45rem', color:'rgba(255,255,255,0.5)', letterSpacing:'0.1em' }}>{connected ? 'LIVE' : 'OFFLINE'}</span>
            </div>

            <button onClick={() => setShowOnline(s => !s)} style={{ display:'flex', alignItems:'center', gap:4, padding:'4px 8px', background:'rgba(201,168,76,0.08)', border:'1px solid rgba(201,168,76,0.2)', borderRadius:3, cursor:'pointer', color:'rgba(201,168,76,0.7)', transition:'all 0.2s' }}
              onMouseEnter={e => e.currentTarget.style.background='rgba(201,168,76,0.15)'}
              onMouseLeave={e => e.currentTarget.style.background='rgba(201,168,76,0.08)'}>
              <Users size={12} />
              <span style={{ fontFamily:"'Bebas Neue', sans-serif", fontSize:'0.6rem', letterSpacing:'0.2em' }}>{onlineUsers.length}</span>
            </button>

            <button onClick={() => setShowBgPicker(true)} style={{ display:'flex', alignItems:'center', gap:4, padding:'4px 8px', background:'rgba(201,168,76,0.08)', border:'1px solid rgba(201,168,76,0.2)', borderRadius:3, cursor:'pointer', color:'rgba(201,168,76,0.7)', transition:'all 0.2s' }}
              onMouseEnter={e => e.currentTarget.style.background='rgba(201,168,76,0.15)'}
              onMouseLeave={e => e.currentTarget.style.background='rgba(201,168,76,0.08)'}>
              <ImageIcon size={12} />
              <span style={{ fontFamily:"'Bebas Neue', sans-serif", fontSize:'0.6rem', letterSpacing:'0.2em' }}>BG</span>
            </button>

            <div style={{ display:'flex', alignItems:'center', gap:5 }}>
              <Avatar name={displayName} size={26} />
              <span style={{ fontFamily:"'Special Elite', monospace", fontSize:'0.68rem', color:'rgba(255,255,255,0.55)', maxWidth:70, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{displayName}</span>
              <button onClick={handleLogout} title="Logout"
                style={{ background:'none', border:'1px solid rgba(192,57,43,0.3)', borderRadius:3, cursor:'pointer', color:'#e74c3c', padding:'3px 5px', display:'flex', alignItems:'center', transition:'all 0.2s' }}
          ...
          <div style={{ position:'relative', zIndex:1, display:'flex', flexDirection:'column', gap:10 }}>
            ...
            {!histLoaded && (
              <div style={{ textAlign:'center', padding:'20px 0' }}>
                <div style={{ width:20, height:20, border:'2px solid #c9a84c', borderTopColor:'transparent', borderRadius:'50%', animation:'spin 0.8s linear infinite', margin:'0 auto 8px' }} />
                <p style={{ fontFamily:'monospace', fontSize:'0.55rem', color:'rgba(201,168,76,0.4)', letterSpacing:'0.2em' }}>LOADING HISTORY...</p>
              </div>
            )}

            {histLoaded && messages.length === 0 && (
              <div style={{ textAlign:'center', marginTop:60 }}>
                <div style={{ fontFamily:"'Bebas Neue', sans-serif", fontSize:'2rem', letterSpacing:'0.2em', color:'rgba(201,168,76,0.15)' }}>🎬</div>
                <p style={{ fontFamily:'monospace', fontSize:'0.6rem', color:'rgba(255,255,255,0.25)', letterSpacing:'0.15em' }}>Be the first to say something!</p>
              </div>
            )}

            {messages.map((msg, i) => (
              <MessageBubble key={msg.id || i} msg={msg} isOwn={msg.uid === user?.uid} />
            ))}
      ...
      <div style={{ background:'#1a1a12', borderTop:'1px solid rgba(201,168,76,0.12)', padding:'10px 14px', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', gap:6, alignItems:'flex-end', position:'relative' }}>
          ...
          <button onClick={() => setShowEmoji(s => !s)}
            style={{ width:36, height:36, flexShrink:0, borderRadius:6, cursor:'pointer', background: showEmoji ? 'rgba(201,168,76,0.18)' : 'rgba(201,168,76,0.08)', border:`1px solid ${showEmoji ? 'rgba(201,168,76,0.45)' : 'rgba(201,168,76,0.18)'}`, display:'flex', alignItems:'center', justifyContent:'center', color:'rgba(201,168,76,0.7)', transition:'all 0.2s' }}>
            <Smile size={16} />
          </button>

          {/* Image upload button */}
          <label style={{ width:36, height:36, flexShrink:0, borderRadius:6, cursor: imgUploading ? 'wait' : 'pointer', background:'rgba(201,168,76,0.08)', border:'1px solid rgba(201,168,76,0.18)', display:'flex', alignItems:'center', justifyContent:'center', color:'rgba(201,168,76,0.7)', transition:'all 0.2s', opacity: imgUploading ? 0.6 : 1 }}
            onMouseEnter={e => !imgUploading && (e.currentTarget.style.background='rgba(201,168,76,0.15)')}
            onMouseLeave={e => (e.currentTarget.style.background='rgba(201,168,76,0.08)')}>
            {imgUploading
              ? <div style={{ width:14, height:14, border:'2px solid #c9a84c', borderTopColor:'transparent', borderRadius:'50%', animation:'spin 0.8s linear infinite' }} />
              : <ImageIcon size={16} />
            }
            <input type="file" accept="image/*" style={{ display:'none' }} onChange={handleImageSend} disabled={imgUploading} />
          </label>
        ...
        <p style={{ fontFamily:'monospace', fontSize:'0.42rem', color:'rgba(255,255,255,0.3)', letterSpacing:'0.1em', marginTop:5, textAlign:'center' }}>
          Enter to send · Shift+Enter new line · 🖼️ Image · 😊 Emoji
        </p>
      </div>

      {/* bottom sprockets */}
      <div style={{ background:'#1a1a12', padding:'5px 0', borderTop:'1px solid rgba(201,168,76,0.08)', flexShrink:0 }}>
        <SprocketRow count={12} />
      </div>
