...
const FontImport = () => (
  <style>{`@import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Special+Elite&display=swap');
...
const Sprockets = ({ count = 7 }) => (
  <div style={{ display:'flex', flexDirection:'column', justifyContent:'space-around', alignItems:'center', padding:'8px 0', height:'100%' }}>
    {Array.from({ length: count }).map((_, i) => (
      <div key={i} style={{ width:14, height:10, background:'#1c1409', border:'1px solid rgba(201,168,76,0.35)', borderRadius:3, margin:'2px 0', flexShrink:0, boxShadow:'inset 0 1px 3px rgba(0,0,0,0.8)' }} />
    ))}
  </div>
);
  ...
  <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
    {[1,2,3].map(i => (
      <div key={i} style={{
        ...
        background:'linear-gradient(90deg, rgba(201,168,76,0.04) 0%, rgba(201,168,76,0.09) 50%, rgba(201,168,76,0.04) 100%)',
...
const FilmAnnouncementCard = ({ item, index }) => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div style={{ position:'fixed', inset:0, zIndex:200, background:'rgba(0,0,0,0.9)', display:'flex', alignItems:'center', justifyContent:'center', padding:'20px', animation:'hmFadeIn 0.25s ease' }} onClick={handleToggle}>
          <div onClick={e => e.stopPropagation()} style={{
            width:'100%', maxWidth:540, maxHeight:'90vh', overflowY:'auto',
            background:'#0a0a08', border:'1px solid rgba(201,168,76,0.6)', borderRadius:4,
            boxShadow:'0 0 80px rgba(201,168,76,0.12), 0 30px 60px rgba(0,0,0,0.9)',
            animation:'hmSlideUp 0.3s cubic-bezier(0.34,1.56,0.64,1)',
            scrollbarWidth:'thin', scrollbarColor:'rgba(201,168,76,0.2) #0a0a08', position:'relative',
          }}>
            ...
            <div style={{ display:'flex' }}>
              <div style={{ width:26, background:'#1a1a15', borderRight:'1px solid rgba(201,168,76,0.12)', flexShrink:0, minHeight:380 }}><Sprockets count={14} /></div>
              <div style={{ flex:1, background:'#141410', overflow:'hidden' }}>
                <div style={{ width:'100%', position:'relative', background:'#0a0a08' }}>
                  <img src={item.imageUrl} alt={item.title} style={{ width:'100%', display:'block', maxHeight:'55vh', objectFit:'contain', filter:'sepia(0.1) contrast(1.05)' }} />
                ...
                <div style={{ padding:'18px 20px 28px' }}>
                  ...
                  <h3 style={{ fontFamily:"'Bebas Neue', sans-serif", fontSize:'1.9rem', letterSpacing:'0.1em', lineHeight:1.05, color:'#e8c97a', marginBottom:12, textShadow:'0 0 30px rgba(201,168,76,0.3)' }}>{item.title}</h3>
              ...
              <div style={{ width:26, background:'#1a1a15', borderLeft:'1px solid rgba(201,168,76,0.12)', flexShrink:0 }}><Sprockets count={14} /></div>
            </div>
          </div>
        </div>
      )}
...
const FilmWatchButton = ({ href }) => {
  ...
  return (
    <a href={href} target="_blank" rel="noreferrer" onClick={handleClick}
      style={{ display:'inline-flex', alignItems:'center', gap:0, flexShrink:0, textDecoration:'none', position:'relative' }}>
      <div style={{ display:'flex', flexDirection:'column', gap:3, padding:'4px 3px', background:'#1a1208', border:'1px solid rgba(201,168,76,0.35)', borderRight:'none', borderRadius:'3px 0 0 3px' }}>
        {[0,1,2].map(i => <div key={i} style={{ width:6, height:4, background:'#1c1409', border:'1px solid rgba(201,168,76,0.3)', borderRadius:1 }} />)}
      </div>
      <div style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 12px', background:'#c9a84c', color:'#0a0a08', transition:'background 0.2s' }}
        onMouseEnter={e => e.currentTarget.style.background='#e8c97a'}
        onMouseLeave={e => e.currentTarget.style.background='#c9a84c'}>
        <svg viewBox="0 0 18 18" width="14" height="14" style={{ transform: spinning ? 'rotate(60deg)' : 'rotate(0deg)', transition:'transform 0.3s' }}>
          ...
          {[0,60,120,180,240,300].map((deg,i) => { const r=deg*Math.PI/180; return <circle key={i} cx={9+5.5*Math.cos(r)} cy={9+5.5*Math.sin(r)} r="1.4" fill="#0a0a08"/>; })}
        </svg>
...
const SectionHeader = ({ icon, title }) => (
  <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:10, marginBottom:20 }}>
    ...
    <h3 style={{ display:'flex', alignItems:'center', gap:8, fontFamily:"'Bebas Neue', sans-serif", fontSize:'1.2rem', letterSpacing:'0.3em', color:'#c9a84c', textTransform:'uppercase', whiteSpace:'nowrap', margin:0 }}>
      {icon} {title}
    </h3>
...
const EmptyState = ({ text }) => (
  <div style={{ textAlign:'center', padding:'32px 20px', border:'1px dashed rgba(201,168,76,0.1)', borderRadius:4 }}>
    <div style={{ fontSize:'1.6rem', marginBottom:8, opacity:0.3 }}>🎞️</div>
    <p style={{ fontFamily:'monospace', fontSize:'0.65rem', color:'rgba(255,255,255,0.2)', letterSpacing:'0.2em', textTransform:'uppercase', margin:0 }}>{text}</p>
  </div>
);
...
const Spinner = () => (
  <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:10, padding:'32px 0' }}>
    <div style={{ width:18, height:18, border:'2px solid rgba(201,168,76,0.2)', borderTopColor:'#c9a84c', borderRadius:'50%', animation:'homeSpin 0.8s linear infinite' }} />
    <span style={{ fontFamily:'monospace', fontSize:'0.6rem', letterSpacing:'0.3em', color:'rgba(201,168,76,0.4)', textTransform:'uppercase' }}>Loading...</span>
  </div>
);
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      setUpdates(snap.docs.map(d => ({ ...d.data(), id: d.id })));
      setLoading(false); // data এলে loading শেষ
    });
    return () => unsub();
  }, []);
      ...
      <main className="relative z-10 flex-grow w-full max-w-[560px] mx-auto p-4 md:p-8 space-y-12">
        ...
        <header className="text-center py-8">
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:12, marginTop:10 }}>
            ...
            <p style={{ fontFamily:"'Bebas Neue', sans-serif", fontSize:'0.75rem', letterSpacing:'0.55em', color:'rgba(201,168,76,0.6)', textTransform:'uppercase' }}>Official Dashboard</p>
        ...
        <section style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(201,168,76,0.12)', borderRadius:20, padding:'28px 28px' }}>
          <SectionHeader icon={<Megaphone size={18}/>} title="Latest News" />
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {loading ? (
              <LoadingSkeleton />
            ) : announcements.length === 0 ? (
              <EmptyState text="No news published yet" />
            ) : (
        ...
        <section style={{ paddingBottom:80 }}>
          <div style={{ display:'flex', flexDirection:'column', gap:14, marginBottom:24 }}>
            <SectionHeader icon={<LayoutGrid size={18}/>} title="My Works" />

            {/* Tab filters */}
            <div style={{ display:'flex', justifyContent:'center' }}>
              <div style={{ display:'flex', background:'#0f0f0d', border:'1px solid rgba(201,168,76,0.2)', borderRadius:4, overflow:'hidden' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {loading ? (
              ...
              <EmptyState text="No works published yet" />
            ) : (
              works.map((item) => (
